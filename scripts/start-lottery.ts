...
import { isTimeToRun, getEndTime, getTicketPrice,getTicketTVSPrice } from "../utils";
...
const main = async () => {
  ...
  if (networkName === "testnet" || networkName === "mainnet") {
    ...
    if (timeToRun) {
      ...
      const TVS_PRICE = await getTicketTVSPrice();
      const TICKET_VALUE = config.Ticket.Price.mainnet;
      const ticketPrice = (TICKET_VALUE/TVS_PRICE).toFixed(0);

      console.log(" TVS PRICE -> ",TVS_PRICE);
      console.log(" TICKET VALUE -> ",TICKET_VALUE);
      console.log(" Ticket Price TVS-> ",ticketPrice);
      ...
      if(TVS_PRICE>0){
        ...
        const message = `[${new Date().toISOString()}] network=${networkName} message='No se pudo obtener el precio del token TVS' signer=${
          operator.address
        }`;
        console.error(message);
        logger.error({ message });    
